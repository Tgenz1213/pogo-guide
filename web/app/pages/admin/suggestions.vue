  ...
  <div class="max-w-6xl mx-auto p-6">
    <div class="mb-6">
      ...
      <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
        Review edit suggestions submitted by users.
      </p>
    </div>
    ...
    <div
      v-else-if="!data?.suggestions?.length"
    ...
    >
      <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
          ...
          >
            <!-- Guide doc Path (truncated) -->
            <td
              class="px-6 py-4 whitespace-nowrap text-sm font-mono text-slate-500 dark:text-slate-400"
            >
              <span :title="suggestion.guidePath">{{
                suggestion.guidePath.length > 20
                  ? suggestion.guidePath.slice(0, 20) + "…"
            ...
            >
              <button
                ...
                @click.stop="performAction(suggestion._id, 'reviewed')"
              >
                Mark Reviewed
              </button>
              <button
                ...
                @click.stop="performAction(suggestion._id, 'dismissed')"
              >
                Dismiss
              </button>
            </td>
...
import { ref } from "vue";
import type { SuggestionRow } from "~/components/admin/AdminSuggestionDetailModal.vue";
...
const selectedSuggestion = ref<SuggestionRow | null>(null);
...
async function performAction(
  ...
  if (!confirm(`Are you sure you want to ${label} this suggestion?`)) return;

  try {
    await $fetch("/api/admin/suggestions/action", {
      method: "POST",
      body: { suggestionId, action },
    });
    refresh();

    if (selectedSuggestion.value?._id === suggestionId) {
...
function handleModalAction(
  ...
  performAction(suggestionId, action);
}
</script>
